...
2. Убедитесь, что в папке `/www/takto.space` есть:
   - ✅ `index.html`
   - ✅ `.htaccess`
   - ✅ `favicon.svg`
   - ✅ `robots.txt`
...
На REG.Ru корневая папка для домена `takto.space`:
- `/www/takto.space` (используется в проекте)

**Если нужно изменить:**
Обновите `REMOTE_DIR` в `deploy-ftp.js`:
```javascript
const REMOTE_DIR = '/www/takto.space'; // или другое название
```
...
1. Убедитесь, что файл `.htaccess` загружен в корень `/www/takto.space`
...
**Решение:**
1. Проверьте, что `index.html` загружен в `/www/takto.space`
...
**Примечание:** После тестирования удалите файл `test.html` с сервера.
